...
\newcommand{\epyfigurewidth}{.333\linewidth}
...
\verb|int[n,n]|. A multidimensional array of doubles with rank $d$ and shape vector $shp$ can be
...
clock of \qty{2.60}{\giga\hertz}. The CPU has \qty{16}{\mega\byte} of L3 cache, and the system itself provides \qty{32}{\giga\byte} of RAM.

\begin{figure*}
  \tiny
  \hfill
  % Chart showing energy consumption and runtime across different numbers of threads. The x-axis represents the number of threads from 1 to 16. The left y-axis shows bars indicating energy consumption in joules. The right y-axis shows an orange line indicating the runtime in seconds. Energy consumption peaks at two threads, and steadily decreases until roughly eight threads, after which it plateaus and reaches a minimum at 16 threads. Runtime peaks at two threads, and gradually decreases until it reaches the minimum at 16 threads.
  \input{img_nbody_10000.tex}%
  \hfill
  % Chart showing energy consumption and runtime across different numbers of threads. The x-axis represents the number of threads from 1 to 16. The left y-axis shows bars indicating energy consumption in joules. The right y-axis shows an orange line indicating the runtime in seconds. Energy consumption peaks at one thread, and steadily decreases until roughly eight threads, after which it plateaus and reaches a minimum at 14 threads. Runtime peaks at one thread, and gradually decreases until it plateaus around 14 threads.
  \input{img_nbody_25000.tex}%
  \hfill
  % Chart showing energy consumption and runtime across different numbers of threads. The x-axis represents the number of threads from 1 to 16. The left y-axis shows bars indicating energy consumption in joules. The right y-axis shows an orange line indicating the runtime in seconds. Energy consumption peaks at one thread, and steadily decreases until roughly eight threads, after which it plateaus and reaches a minimum at 12 threads. Runtime peaks at one thread, and gradually decreases until it plateaus around 12 threads.
...
For an N-body simulation with $10000$ elements we observe that although
increasing the thread-count strictly decreases the runtime, for simulations with $25000$ and $40000$
elements, the energy consumption
...
\begin{figure*}
  ...
  \hfill
  % Chart showing energy consumption and runtime across different numbers of threads. The x-axis represents the number of threads from 1 to 16. The left y-axis shows bars indicating energy consumption in joules. The right y-axis shows an orange line indicating the runtime in seconds. Energy consumption peaks at two threads, and steadily decreases until roughly eight threads, after which it plateaus and reaches a minimum at 14 threads. Runtime peaks at two threads, and gradually decreases until it plateaus around 14 threads.
  \input{img_stencil_25000.tex}%
  \hfill
  % Chart showing energy consumption and runtime across different numbers of threads. The x-axis represents the number of threads from 1 to 16. The left y-axis shows bars indicating energy consumption in joules. The right y-axis shows an orange line indicating the runtime in seconds. Energy consumption peaks at two threads, and steadily decreases until roughly eight threads, after which it plateaus and reaches a minimum at 12 threads. Runtime peaks at two threads, and gradually decreases until it plateaus around 12 threads.
...
For a $10000 \times 10000$ input matrix we observe that both the
...
both the energy-efficiency and runtime performance. Whereas the input size increases, the optimum thread-count
decreases.

\begin{figure*}
  \tiny
  \hfill
  % Chart showing energy consumption and runtime across different numbers of threads. The x-axis represents the number of threads from 1 to 16. The left y-axis shows bars indicating energy consumption in joules. The right y-axis shows an orange line indicating the runtime in seconds. Energy consumption peaks at one threads and gradually decreases from one to eight threads, after which it starts noticeably fluctuating. Reaching a minimum at 14 threads, with an energy consumption roughly equal to that of eight threads. Runtime follows a similar pattern, albeit being more pronounced.
  \input{img_matmul_500.tex}%
  \hfill
  % Chart showing energy consumption and runtime across different numbers of threads. The x-axis represents the number of threads from 1 to 16. The left y-axis shows bars indicating energy consumption in joules. The right y-axis shows an orange line indicating the runtime in seconds. Energy consumption gradually decreases from one to eight threads, reaching a minimum at eight threads. Going to nine threads there is a significant jump in energy consumption, peaking at nine threads and gradually decreasing until 16 threads. The energy consumption of 16 threads is above that of one thread. Runtime follows a similar pattern, albeit being more pronounced.
...
The results of a $500 \times 500$ matrix multiplication in Figure~\ref{fig:matmul} align with our
...
thread-count of eight. However, increasing the matrix size to $1000 \times 1000$ and $1500 \times 1500$ changes this energy
...
\begin{figure*}
  \tiny
  \hfill
  % Chart showing energy consumption and runtime across different numbers of threads. The x-axis represents the number of threads from 1 to 16. The left y-axis shows bars indicating energy consumption in joules. The right y-axis shows an orange line indicating the runtime in seconds. Energy consumption peaks at one threads, and steadily decreases until roughly eight threads, after which it plateaus and reaches a minimum at 16 threads. Runtime follows the same pattern.
  \input{img_rust_500.tex}%
  \hfill
  % Chart showing energy consumption and runtime across different numbers of threads. The x-axis represents the number of threads from 1 to 16. The left y-axis shows bars indicating energy consumption in joules. The right y-axis shows an orange line indicating the runtime in seconds. Energy consumption peaks at one threads, and steadily decreases until roughly eight threads, after which it plateaus and reaches a minimum at 15 threads. Runtime follows the same pattern.
  \input{img_rust_1000.tex}%
  \hfill
  % Chart showing energy consumption and runtime across different numbers of threads. The x-axis represents the number of threads from 1 to 16. The left y-axis shows bars indicating energy consumption in joules. The right y-axis shows an orange line indicating the runtime in seconds. Energy consumption peaks at one threads, and steadily decreases until it reaches a minimum at eight threads, after which energy consumption starts gradually increasing again. Runtime follows the same pattern.
...
significantly different energy consumption pattern. For the $500 \times 500$ and $1000 \times 1000$ cases,
...
\begin{figure}[t]
  ...
  \begin{minipage}[c]{.5\linewidth}
    \centering
    % Line chart showing the number of threads used during program runtime against of the N-body simulation. The x-axis represents the number of elements, labeled as $10000$, $25000$, and $40000$. The y-axis shows the number of threads, ranging from 1 to 16. An orange dashed line represents the optimal thread count, and a blue line indicates the actual thread count. When the input size changes the blue line oscillates a lot for a few iterations. Afterwards it oscillates by less than one thread around the optimum at the orange line.
  ...
  \begin{minipage}[c]{.5\linewidth}
    \centering
    % Line chart showing the number of threads used during program runtime of the matrix multiplication algorithm. The x-axis represents matrix size, labeled as $500 \times 500$, $1000 \times 1000$, and $1500 \times 1500$. The y-axis shows the number of threads, ranging from 1 to 16. An orange dashed line represents the optimal thread count, and a blue line indicates the actual thread count. When the input size changes the blue line oscillates a lot for a few iterations. Afterwards, for the $500 \times 500$ input, it oscillates by less than one thread around the optimum at the orange line. For the other two input sizes it converges to four threads, instead of the optimum at the orange line at eight threads.
...
\begin{figure}[t]
  ...
  \begin{minipage}[c]{.5\linewidth}
    \centering
    % Line chart showing the number of threads used during program runtime of the nine-point stencil operation. The x-axis represents matrix size, labeled as $10000 \times 10000$, $25000 \times 25000$, and $40000 \times 40000$. The y-axis shows the number of threads, ranging from 1 to 16. An orange dashed line represents the optimal thread count, and a blue line indicates the actual thread count. When the input size changes the blue line oscillates a lot for a few iterations. Afterwards it oscillates by less than one thread around the optimum at the orange line. For the $40000 \times 4000$ input it converges to ten threads, instead of the optimum at the orange line at twelve threads.
  ...
  \begin{minipage}[c]{.5\linewidth}
    \centering
    % Line chart showing the number of threads used during program runtime of the Rust implementation of the matrix multiplication algorithm. The x-axis represents matrix size, labeled as $500 \times 500$, $1000 \times 1000$, and $1500 \times 1500$. The y-axis shows the number of threads, ranging from 1 to 16. An orange dashed line represents the optimal thread count, and a blue line indicates the actual thread count. When the input size changes the blue line oscillates a lot for a few iterations. Afterwards it oscillates by less than one thread around the optimum at the orange line.
...
instead of twelve threads, the energy consumption at these two thread-counts are within margin of error.
...
\begin{table*}[ht]
  ...
  \begin{subtable}{0.33\linewidth}
    ...
    \begin{tabular}{ccc}\toprule
      ...
      $40000^2$ & \qty{15}{\percent} & \qty{33}{\percent} \\\bottomrule
    \end{tabular}%
  ...
  \begin{subtable}{0.33\linewidth}
    ...
    \begin{tabular}{ccc}\toprule
      ...
      $1500^2$ & \qty{68}{\percent} & \qty{100}{\percent} \\\bottomrule
    \end{tabular}%
  ...
  \begin{subtable}{0.33\linewidth}
    ...
    \begin{tabular}{ccc}\toprule
      ...
      $1500^2$ & \qty{10}{\percent} & \qty{16}{\percent} \\\bottomrule
    \end{tabular}%
...
a similar level of performance compared to the oracle, with only a \qty{5}{\percent} increase in energy
consumption. However, for $40000 \times 40000$ inputs the energy consumption is increased by \qty{15}{\percent}
...
it does have significantly worse runtime, leading to a \qty{33}{\percent} increase in runtime. The decrease in
...
additional energy requirements due to an increase in runtime, a \qty{15}{\percent} increase in energy consumption
compared to a \qty{33}{\percent} increase in runtime seems reasonable.
...
inputs we are within \qty{8}{\percent} of the energy consumption and runtime of the oracle-based approach.
...
case where the dynamic approach has an \qty{18}{\percent} longer runtime, the increase in energy consumption is
only \qty{8}{\percent}. In the other two cases, the energy-efficiency is within margin of error of the oracle.
...
\begin{table*}[ht]
  ...
  \begin{subtable}{0.33\linewidth}
    ...
    \begin{tabular}{ccc}\toprule
      ...
      $40000^2$ & \qty{7}{\percent}  & \qty{25}{\percent} \\\bottomrule
    \end{tabular}%
    \caption{Nine-point stencil.}%
  ...
  \begin{subtable}{0.33\linewidth}
    ...
    \begin{tabular}{ccc}\toprule
      ...
      $1500^2$ & \qty{-3}{\percent} & \qty{14}{\percent} \\\bottomrule
    \end{tabular}%
    \caption{Matrix multiplication.}%
    \label{tab:rt-matmul}
  \end{subtable}%
  \begin{subtable}{0.33\linewidth}
    ...
    \begin{tabular}{ccc}\toprule
      ...
      $1500^2$ & \qty{1}{\percent}  & \qty{-2}{\percent} \\\bottomrule
    \end{tabular}%
    \caption{Rust implementation.}%
    \label{tab:rt-rust}
  \end{subtable}%
...
However, the energy-optimising approach does have a \qty{25}{\percent} greater runtime than the runtime-optimising
...
compared to the stark increase in runtime, a \qty{7}{\percent} increase in energy consumption seems reasonable.
...
\begin{table*}[ht]
  ...
  \begin{subtable}{0.33\linewidth}
    ...
    \begin{tabular}{ccc}\toprule
      ...
      16 & \qty{-5}{\percent}  & \qty{7}{\percent} \\\bottomrule
    \end{tabular}%
    \caption{Nine-point stencil.}%
  ...
  \begin{subtable}{0.33\linewidth}
    ...
    \begin{tabular}{ccc}\toprule
      ...
      16 & \qty{-49}{\percent} & \qty{-29}{\percent} \\\bottomrule
    \end{tabular}%
    \caption{Matrix multiplication.}%
  ...
  \begin{subtable}{0.33\linewidth}
    ...
    \begin{tabular}{ccc}\toprule
      ...
      16 & \qty{-7}{\percent}   & \qty{-7}{\percent} \\\bottomrule
    \end{tabular}%
    \caption{Rust implementation.}%
...
choice in thread-count is made for the nine-point stencil operation we observe that \qty{10}{\percent} of energy
can be saved with our dynamic approach. Compared to the single-threaded case, our dynamic approach
consumes \qty{79}{\percent} less energy on average. Statically choosing twelve threads leads to a slightly lower
...
poor choice is made for the static thread-count, we save around \qty{50}{\percent} of energy, even without an
...
consumption~\cite{compiler-energy-differences}. Pallister et~al. provide several optimizations
that aim to reduce the energy consumption of programs, in the context of embedded
devices~\cite{compiler-energy-flash,compiler-energy-methods}. They also find that for
...
Bangash et~al. investigate bytecode transformations in the context of Android applications,
determining whether certain (combinations of) transformations lead to in increase or decrease in
energy consumption~\cite{compiler-energy-android}. They find that certain combinations of bytecode
transformations can actually increase energy consumption, whereas choosing the right combination of
transformation can lead to a reduction of up to \qty{11}{\percent} in energy consumption.
...
adoption of such techniques have been previously investigated~\cite{strategies-hpc,strategies-industry}.
...
the hardware. Kistowski et~al. compare a variety of load distribution strategies, and propose
...
most cases gets within \qty{15}{\percent} of a manually generated, ideally adapting runtime. The only remarkable
...
Depending on the number of threads chosen, the gains are up to \qty{79}{\percent} for any reasonable choice, and
up to \qty{100}{\percent} if a poor decision is made. We also notice that in many cases our energy-optimising
