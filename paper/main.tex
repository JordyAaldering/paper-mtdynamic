...
A notable approach in the context of the Single assignment C (SaC) language uses a dynamic adaptation algorithm that adjusts the thread-count of a program by observing changes in runtime metrics~\cite{gordon2015dynamic}.
...
(Section~\ref{sec:evaluation-runtime} and
  \ref{sec:evaluation-static})
\end{itemize}

\section{Single assignment C}
Single assignment C (SaC) is a functional array language that resembles the imperative syntax of languages such as C, whilst remaining side-effect-free~\cite{grelck2006sac,grelck2012single}.
...
It constitutes an index vector \texttt{iv} that ranges over the index set with lower bound \texttt{[0,0]} and upper bound \texttt{[9,9]}.
If the lower or upper bound can be inferred from the usage of \texttt{iv} in the inner expression, they may be left out.
In the example, the upper bound would be inferred as the shape of \texttt{arr}.
For each index vector in this range, the result is a selection into the array \texttt{arr} incremented by one.
...
For example, an integer vector of length $100$ can be defined as \texttt{int[100]}, and a square integer matrix of size $n \times n$ can be referred to as \texttt{int[n,n]}.
A multidimensional array of doubles with rank $d$ and shape vector $shp$ can be described by \texttt{double[d:shp]}.
...
Furthermore, shape variables such as \texttt{u}, \texttt{v}, and \texttt{w} can be used in function bodies as if they are user-defined variables.
...
On Apple Silicon, for example, this is possible through the use of the \texttt{task\_info} API, which returns a per-process energy consumption value~\cite{queze2023power}.
For Apple's Intel-based machines, the \texttt{diagCall64} function can be used instead.
...
The system is accessible through Slurm, and contains a single 8 core 16 thread CPU, running at a base clock of \qty{2.60}{\giga\hertz}.
...
\begin{figure}[!t]
  ...
  \end{subfigure}%
  \caption{Median energy consumption and runtime of 250 N-body simulation in SaC.
  Bars denote energy consumption, lines denote runtime.
  Errors are reported as the interquartile range.}%
  \label{fig:nbody}
\end{figure}
...
A single time step of the N-body simulation with delta-time \texttt{dt} on an array of bodies is defined as follows, where \texttt{acc} is a function that returns the acceleration vector between two bodies.
\begin{verbatim}
...
For each thread count, bars denote the median energy consumption of a single time step and the line denotes the median runtime.

For an N-body simulation with $10000$ elements we observe that although increasing the thread-count strictly decreases the runtime, for simulations with $25000$ and $40000$ elements, the energy consumption plateaus when using more than eight threads.
...
\begin{figure}[!t]
  ...
  \end{subfigure}%
  \caption{Median energy consumption and runtime of 250 nine-point stencil operations in SaC.
  Bars denote energy consumption, lines denote runtime.
  Errors are reported as the interquartile range.}%
  \label{fig:stencil}
\end{figure}
...
\begin{figure}[!t]
  ...
  \end{subfigure}%
  \caption{Median energy consumption and runtime of 250 naive matrix multiplications in SaC.
  Bars denote energy consumption, lines denote runtime.
  Errors are reported as the interquartile range.}%
  \label{fig:matmul}
\end{figure}
...
\begin{figure}[!t]
  ...
  \end{subfigure}%
  \caption{Median energy consumption and runtime of 250 naive matrix multiplications in Rust.
  Bars denote energy consumption, lines denote runtime.
  Errors are reported as the interquartile range.}%
  \label{fig:rust}
\end{figure}
...
The implementation of the thread-switching beehive system and adaptation algorithm in SaC are available on the SaC GitLab project~\cite{mtdynamic2024repository}.
...
Compared to the single-threaded case, our dynamic approach consumes \qty{79}{\percent} less energy on average.
...
Although there exists a myriad of compiler optimizations that aim to minimize the runtime of programs, not many optimizations exist that aim to specifically reduce energy consumption~\cite{Pallister2016exploring}.
...
One of the key requirements in allowing developers to increase the energy-efficiency of their software is the ability to visualize and analyse the energy consumption of their code~\cite{steen2023organizational}.
