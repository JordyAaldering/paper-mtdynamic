...
Determining efficient resource allocation for optimising performance\---be it runtime or energy efficiency\---depends not only on an algorithm's implementation, but also on input data size, hardware characteristics, cache utilisation, and system\-/specific configurations such as thread pinning.
...
Furthermore, it has been shown that optimising for runtime is not always equivalent to optimising for energy efficiency, and that optimising runtime can have a negative effect on energy consumption~\cite{bangash2022black,pallister2016exploring}.
...
This technique enables SaC applications to adapt to varying workloads and resource availability, improving runtime performance by efficiently utilising computational resources.
...
Before the actual generation of multithreaded code, the high\-/level optimisation phase of the compiler extensively fuses tensor comprehensions to increase the granularity of parallelism and to improve the locality of code~\cite{grelck2006sac}.
...
Our aim is to determine whether there are cases where using all available resources is not optimal, and whether optimising for energy efficiency always correlates with optimising for runtime.
...
The runtime and energy measurements start right before those parallel regions, and stop immediately after their synchronisation.
...
This suggests that the overhead caused by having to manage multiple threads has a stronger negative effect on the energy consumption of a program than on its runtime performance, and that optimising for runtime performance has diminishing returns for decreasing energy consumption.
...
\begin{figure}[ht]
  ...
  \end{subfigure}%
  \caption{Average energy consumption and runtime of \qty{250} nine\-/point stencil operations in SaC.
  Bars denote energy consumption, with corresponding values in the left y\-/axis.
  The line denotes runtime, corresponding to the right y\-/axis.}%
  \label{fig:stencil}
\end{figure}
...
For a $\num{10000} \times \num{10000}$ input matrix we observe that both the runtime and energy consumption decrease as the number of threads increases.
However, for $\num{25000} \times \num{25000}$ and $\num{40000} \times \num{40000}$ inputs, using all available threads is no longer optimal for both the energy efficiency and runtime performance.
...
The results of a $\num{500} \times \num{500}$ matrix multiplication in Figure~\ref{fig:matmul} align with our expectations; increasing the number of threads up to eight threads improves both the energy efficiency and the runtime performance.
...
However, increasing the matrix size to $\num{1000} \times \num{1000}$ and $\num{1500} \times \num{1500}$ changes this energy consumption pattern, resulting from the shift of being CPU\-/bound to being memory\-/bound.
...
For the $\num{500} \times \num{500}$ and $\num{1000} \times \num{1000}$ cases, we observe that up until eight threads, increasing the thread count improves both the energy efficiency and runtime.
...
As the matrix size increases to $\num{1500} \times \num{1500}$, there is a shift in the energy consumption pattern.
As the number of threads increases from 8 to 16, the energy consumption gradually increases again.
This suggests that the overhead associated with multithreading outweighs the benefits of further parallelisation.
...
Although we suspect that these are the main reasons for this performance difference, it is unclear why exactly this behaviour occurs, complicating the task of determining an optimal thread count and emphasising the need for a dynamic approach.
...
We repeat the same benchmark as in Section~\ref{sec:evaluation-oracle}, however we now compare the energy\-/based dynamic adaptation algorithm against an implementation of the dynamic runtime\-/optimising algorithm~\cite{gordon2015dynamic}.
...
Interestingly, for $1500 \times 1500$ we observe that the energy\-/optimising approach decreases energy consumption, but increases runtime, showing that optimising for energy consumption is not necessarily equivalent to optimising for runtime.
...
By synchronising messages in a queue, it aims to reduce the overhead caused by acquiring and releasing synchronisation primitives, as well as preventing busy waiting within handlers.
...
\paragraph{Profile-based optimisation}
Profile\-/based optimisations use the results of profiling runs to select an optimal implementation of an algorithm for the given input or hardware configuration.
...
It would be interesting to investigate whether similar results can be obtained by optimising for energy efficiency instead.

Profile\-/based optimisation of virtual machine scheduling is a popular avenue of research in the context of data\-/centres~\cite{ding2017profile,buyya2010energy,dai2016energy,minarolli2011utility}.
...
\paragraph{Static methods for optimising energy}
Although there exists a myriad of compiler optimisations that aim to minimise the runtime of programs, not many optimisations exist that aim to specifically reduce energy consumption~\cite{pallister2016exploring}.
Several optimisations aim to reduce the energy consumption of programs, in the context of embedded devices~\cite{pallister2015optimizing,pallister2013identifying}.
...
In the context of Android applications, bytecode transformations have been used to determining whether certain (combinations of) transformations lead to an increase or decrease in energy consumption~\cite{bangash2022black}.
...
\paragraph{Energy visualisation \& analysis}
...
\paragraph{Energy-optimising strategies}
...
Related work compares a variety of load distribution strategies, and propose a new strategy that reduces a system's energy consumption even further~\cite{kistowski2015energy}.
...
However, we also find that optimising these cases is highly context\-/dependent and that slight variations in configuration or implementation can introduce significant changes to the energy consumption characteristics of a program.
