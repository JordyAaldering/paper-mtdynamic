...
For example, an integer vector of length \num{100} can be defined as \code{int[100]}, and a square integer matrix of size $n \times n$ can be referred to as \code{int[n,n]}.
...
The system is accessible through Slurm, and contains a single \num{8} core \num{16} thread CPU, running at a base clock of \qty{2.60}{\giga\hertz}.
...
For an N-body simulation with \num{10000} elements we observe that although increasing the thread count strictly decreases the runtime, for simulations with \num{25000} and \num{40000} elements, the energy consumption plateaus when using more than eight threads.
...
\begin{figure}[ht]
  ...
  \end{subfigure}%
  \caption{Average energy consumption and runtime of \num{250} nine\-/point stencil operations in SaC.
  Bars denote energy consumption, with corresponding values in the left y\-/axis.
  The line denotes runtime, corresponding to the right y\-/axis.}%
  \label{fig:stencil}
\end{figure}
